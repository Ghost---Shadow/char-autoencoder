...
class Seq2SeqAutoencoder:
    def __init__(
        ...
        with tf.name_scope("process_input"):
            self.x = tf.squeeze(
                tf.one_hot(self.x_, self.input_size, axis=2), axis=3, name="x_one_hot"
            )
        ...
        with tf.name_scope("prediction_vars"):
            self.W1 = tf.Variable(
                tf.random_normal([self.state_size, self.state_size // 2])
            )
            self.W1 = tf.expand_dims(self.W1, 0)
            self.B1 = tf.Variable(tf.random_normal([self.state_size // 2]))

            self.W2 = tf.Variable(
                tf.random_normal([self.state_size // 2, self.input_size])
            )
        ...
        # Text summary
        text_init = np.squeeze(
            np.array([[[""] * (self.interp_steps + 1)] * (self.interp_steps + 1)])
        )
        # text_init = ['','','']
        self.text_variable = tf.Variable(text_init, name="text_variable")
        tf.summary.text(
            "text_summary", tf.convert_to_tensor(self.text_variable, dtype=tf.string)
        )
    ...
    def generate_text_summary(self, sess, strings):
        ...
        with tf.name_scope("text_summary"):
            ...
            resultStrings = np.reshape(
                np.array(resultStrings), [self.interp_steps + 1, self.interp_steps + 1]
            )
    ...
    def eval_and_write_summaries(self, sess, batch, epoch):
        accuracy, loss, summary_output = sess.run(
            [self.accuracy, self.loss, self.summary_op], feed_dict={self.x_: batch}
        )
