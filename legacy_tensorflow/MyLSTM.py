...
class MyLSTM:
    def __init__(self, input_size, state_size):
        ...
        with tf.name_scope("LSTM_Weights"):
            for index in ["i", "f", "o", "g"]:
                self.W[index] = tf.Variable(
                    tf.random_uniform(
                        [input_size, state_size], minval=-0.1, maxval=0.1
                    ),
                    name="W_" + index,
                )
                self.U[index] = tf.Variable(
                    tf.random_uniform(
                        [state_size, state_size], minval=-0.1, maxval=0.1
                    ),
                    name="U_" + index,
                )
                self.B[index] = tf.Variable(
                    tf.random_uniform([state_size], minval=-0.1, maxval=0.1),
                    name="B_" + index,
                )
    ...
    def _unroll(self, x, s, c):
        ...
        for t in range(time_steps):
            with tf.name_scope("unroll_" + str(t)):
                i = tf.nn.sigmoid(
                    tf.matmul(x[t], self.W["i"])
                    + tf.matmul(s[t], self.U["i"])
                    + self.B["i"],
                    name="i",
                )
                f = tf.nn.sigmoid(
                    tf.matmul(x[t], self.W["f"])
                    + tf.matmul(s[t], self.U["f"])
                    + self.B["f"],
                    name="f",
                )
                o = tf.nn.sigmoid(
                    tf.matmul(x[t], self.W["o"])
                    + tf.matmul(s[t], self.U["o"])
                    + self.B["o"],
                    name="o",
                )

                g = tf.nn.tanh(
                    tf.matmul(x[t], self.W["g"])
                    + tf.matmul(s[t], self.U["g"])
                    + self.B["g"],
                    name="g",
                )
    ...
    def decode(self, s, c, time_steps):
        ...
        with tf.name_scope("decoder"):
            ...
            x = tf.unstack(
                tf.zeros([batch_size, time_steps, self.input_size]),
                axis=1,
                name="zero_input",
            )
